...
across all five dashboards. No live break-/fix-*.sh wired up yet, but
the exporter is ready for one.
"""
import http.server
import math
import os
...
def render_metrics():
    ...
    js_error_rate = (6.8 if unhealthy else wave(260, 0.3, 1.4, phase=0.2))
    ...
    for i, err_type in enumerate(["network", "render", "script"]):
        rate = session_rate * (0.03 if unhealthy and err_type == "network" else wave(200, 0.001, 0.01, phase=i))
    ...
    for i, (key, label, retention, base_dwell) in enumerate(FUNNEL_STEPS):
        ...
        dwell = base_dwell * (1.8 if unhealthy and key == "vertragsuebersicht" else 1) + wave(140, -3, 3, phase=i)
    ...
    for i, path in enumerate(HTTP_PATHS):
        ...
        rate = (16 if hot else wave(190, 0.1, 1.6, phase=i * 0.9))
    ...
    for i, svc in enumerate(SERVICES):
        ...
        net_in = wave(100, 5, 40, phase=i) * (2.4 if degraded else 1)
        net_out = wave(100, 8, 55, phase=i + 3) * (2.4 if degraded else 1)
    ...
    load1 = (6.4 if unhealthy else wave(90, 0.8, 2.4, phase=0.2))
    load5 = (5.1 if unhealthy else wave(140, 1.0, 2.2, phase=0.4))
